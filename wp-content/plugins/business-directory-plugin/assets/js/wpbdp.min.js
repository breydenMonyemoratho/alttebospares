"undefined" == typeof window.WPBDP && (window.WPBDP = {}), "undefined" == typeof window.wpbdp && (window.wpbdp = {}), jQuery(function($) {
        ...
        $(".wpbdp-js-select2").length > 0 && $.fn.selectWoo && $(".wpbdp-js-select2").selectWoo(), wpbdp.main_box = {
            init: function() {
                return
            },
        ...
        deleteUpload: function(element_id, element) {
            var $input = (jQuery("#wpbdp-upload-iframe-" + element_id), jQuery('input[name="' + element + '"]')),
                $preview = $input.siblings(".preview");
            return $input.val(""), $preview.find("img").remove(), $preview.find("input").val(""), $preview.hide(), $preview.siblings().show(), !1
        }
    },
    function($) {
        var sb = wpbdp.listingSubmit = {
            ...
            init_events: function() {
                $("#wpbdp-submit-listing").on("click", ".wpbdp-inner-field-option-select_all", function(e) {
                    ...
                    return "Other" !== $(this).val() ? void $icon_element.hide() : void $icon_element.show()
                })
            }
        };
        sb.images = wpbdp.listingSubmit.images = {
            ...
            init: function() {
                ...
                this._admin_nonce || (sb.images._slots = parseInt($("#image-slots-total").text()), sb.images._slotsRemaining = parseInt($("#image-slots-remaining").text())), $("#wpbdp-uploaded-images").on("click", ".wpbdp-image-delete-link", "click", function(e) {
                    ...
                    e.preventDefault();
                    var image_frame, url = $(this).attr("data-action");
                    image_frame && image_frame.open(), image_frame = wp.media({
                        title: "Select Media",
                        multiple: !1,
                        library: {
                            type: "image"
                        ...
                        selection.each(function(attachment) {
                            gallery_ids[i] = attachment.id, i++
                        });
                        var ids = gallery_ids.join(",");
                        ids && $.post(url, {
                            image_ids: ids
                        }, function(res) {
                            if (res.success ? errors = "undefined" != typeof res.data.errors && res.data.errors : errors = [res.data.errors], errors) {
                                var errorMsg = $("<div>").addClass("wpbdp-msg error").html(errors);
                                return void $(res.data.errorElement).prepend(errorMsg)
                            }
                            if ($(res.data.errorElement + " .wpbdp-msg.error").remove(), "listing_field" === res.data.source) {
                                if (!res.data.inputElement) return;
                                var $input = $('input[name="' + res.data.inputElement + '"]');
                                $input.val(res.data.media_id);
                                var $preview = $input.siblings(".preview");
                                return $preview.find("img").remove(), $preview.prepend(res.data.html), $preview.siblings().hide(), void $preview.show()
                            }
                            $("#current-images-header").hide(), $(res.data.previewElement).append(res.data.html)
                        })
                    }), image_frame.open()
                    ...
                    update: function(ev, ui) {
                        ...
                        $.each(sorted, function(i, v) {
                            $('input[name="images_meta[' + v + '][order]"]').val(no_images - i), 0 === i && (document.getElementById("_thumbnail_id").value = v)
                        })
                    }
                })
            }
